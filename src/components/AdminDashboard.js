...
import {
  ...
  CardHeader, CardActions, Stack, Link, Fab, Rating, useMediaQuery
} from '@mui/material';
import {
  Menu as MenuIcon,
...
import { addDummyProducts, resetDummyProducts } from '../utils/dummyProducts';
    ...
    >
      ...
      <DialogContent dividers>
        <Grid container spacing={3}>
          ...
          {/* Order Timeline */}
          {(order.statusHistory?.length > 0 || order.assignedAt || order.pickedAt) && (
            <Grid item xs={12}>
              <Typography variant="subtitle1" gutterBottom color="primary">
                Order Timeline
              </Typography>
              <Paper elevation={1} sx={{ p: 2 }}>
                ...
                <Stack spacing={1.5} sx={{ mb: 2 }}>
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
                    <Chip label="Placed" size="small" variant="outlined" sx={{ minWidth: 100, fontSize: '0.7rem' }} />
                    <Typography variant="caption" sx={{ fontWeight: 500 }}>
                      {formatDate(order.orderDate || order.createdAt) || 'N/A'}
                    </Typography>
                  </Box>
                  {order.assignedAt && (
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
                      <Chip label="Assigned" size="small" color="primary" variant="outlined" sx={{ minWidth: 100, fontSize: '0.7rem' }} />
                      <Typography variant="caption" sx={{ fontWeight: 500 }}>
                        {formatDate(order.assignedAt)}
                        {order.sellerInfo?.email ? ` — Seller: ${order.sellerInfo.email}` : ' — Assigned to seller'}
                      </Typography>
                    </Box>
                  )}
                  {order.pickedAt && (
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
                      <Chip label="Picked Up" size="small" color="secondary" variant="outlined" sx={{ minWidth: 100, fontSize: '0.7rem' }} />
                      <Typography variant="caption" sx={{ fontWeight: 500 }}>
                        {formatDate(order.pickedAt)} — Seller confirmed pick-up
                      </Typography>
                    </Box>
                  )}
                </Stack>
                        ...
                        .map((entry, index) => (
                          <Box key={index} sx={{ display: 'flex', alignItems: 'flex-start', gap: 1.5 }}>
                            <Chip
                              ...
                              sx={{ minWidth: 110, fontSize: '0.65rem', textTransform: 'capitalize' }}
                            />
                            <Box>
...
const AdminDashboard = () => {
  ...
  const handleViewDetails = (order) => {
    setSelectedOrderInTable(order);
    setIsOrderTableModalOpen(true);
  };
          ...
          <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
            ...
            <Box sx={{ display: 'flex', gap: 2, flexWrap: 'wrap' }}>
              ...
              <Button
                ...
                color="warning"
                startIcon={<AddIcon />}
                onClick={handleResetDummyProducts}
                disabled={loading}
                title="Delete old dummy products and re-add with working images"
              >
                🔄 Reset & Fix Images
              </Button>
              <Button
          ...
          <TableBody>
            {sellers.map((seller) => (
              <TableRow key={seller.id}>
                ...
                <TableCell>
                  <Rating
                    value={seller.starRating || seller.rating || 0}
                    readOnly
                    size="small"
                    precision={0.5}
                  />
                ...
                <TableCell>
                  <Chip
                    label={`${seller.creditScore ?? 0} pts`}
                    size="small"
                    sx={{
                      bgcolor: (seller.creditScore || 0) >= 700 ? '#e8f5e9' : (seller.creditScore || 0) >= 400 ? '#fff8e1' : '#ffebee',
                      color: (seller.creditScore || 0) >= 700 ? '#2e7d32' : (seller.creditScore || 0) >= 400 ? '#f57f17' : '#c62828',
                      fontWeight: 700,
                    }}
                  />
                </TableCell>
  ...
  const renderSellersProductsContent = () => {
    // Get all active/frozen sellers for the dropdown (NOT just sellers who have products)
    // This ensures sellers with 0 products still appear in the dropdown
  ...
  const handleResetDummyProducts = async () => {
    try {
      setLoading(true);
      const result = await resetDummyProducts();
      if (result) {
        await fetchProducts();
        setSnackbar({
          open: true,
          message: 'Successfully reset dummy products with fresh images',
          severity: 'success'
        });
      }
    } catch (error) {
      ...
      setSnackbar({
        open: true,
        message: 'Failed to reset dummy products',
        severity: 'error'
      });
    } finally {
      setLoading(false);
  ...
  const handleUpdateSeller = async () => {
    ...
    try {
      ...
      const updatedData = {
        ...
        starRating: selectedSeller.starRating || selectedSeller.rating || 0,
        creditScore: selectedSeller.creditScore ?? 0,
        boostedViews: selectedSeller.boostedViews ?? 0,
        updatedAt: serverTimestamp()
      };
  ...
  return (
<Box className={'relative'} sx={{ display: 'flex', flexDirection: 'column', minHeight: '100vh', mt: 0 }}>

      {/* Mobile hamburger button — hidden on conversations tab (chat has its own nav) */}
        ...
        <Drawer
          sx={{
            ...
            '& .MuiDrawer-paper': {
              ...
              height: isMobile ? '100%' : `calc(100% - ${navbarHeight}px)`,
              overflowY: 'auto',
            },
          }}
          variant={isMobile ? 'temporary' : 'permanent'}
          anchor="left"
          open={isMobile ? toogle : true}
          ...
          <Box sx={{ overflow: 'auto' }}>
            ...
            {isMobile && (
              <Box sx={{ display: 'flex', justifyContent: 'flex-end', px: 1, pt: 1 }}>
                <IconButton onClick={() => setToogle(false)} size="small">
                  <CloseIcon />
                </IconButton>
              </Box>
            )}
...
{/* 
        ...
        <Main open={true} sx={{ 
          ...
          position: 'relative',
          // Conversations tab: fixed height, no scroll (chat handles its own scroll)
          minHeight: activeTab === 'conversations' ? 'unset' : `calc(100dvh - ${navbarHeight}px)`,
          height: activeTab === 'conversations' ? `calc(100dvh - ${navbarHeight}px)` : 'auto',
          background: activeTab === 'conversations'
            ? 'transparent'
            : 'linear-gradient(to bottom, #f5f7ff, #ffffff)',
          overflowY: activeTab === 'conversations' ? 'hidden' : 'auto',
          pb: activeTab === 'conversations' ? 0 : 6,
          pt: (isMobile && activeTab !== 'conversations') ? 6 : 0,
        }}
        onClick={() => {
            ...
            <form ref={formRef}>
              <Grid container spacing={3}>
                ...
                <Grid item xs={12} sm={6}>
                  ...
                  <Box sx={{ mt: 2 }}>
                    <Typography component="legend">Seller Star Rating</Typography>
                    <Rating
                      name="rating"
                  ...
                  <Box sx={{ mt: 2 }}>
                    <TextField
                      ...
                      value={selectedSeller.creditScore ?? 0}
                      onChange={(e) => {
                        const val = Math.min(1000, Math.max(0, Number(e.target.value)));
                        setSelectedSeller(prev => ({ ...prev, creditScore: val }));
                      }}
                      variant="outlined"
                      helperText="Set the seller's credit score. Shown live on their dashboard."
                    />
                  </Box>
                  <Box sx={{ mt: 2 }}>
                    <TextField
                      fullWidth
                      label="Inject Extra Views (Today)"
                      name="boostedViews"
                      type="number"
                      inputProps={{ min: 0 }}
                      value={selectedSeller.boostedViews ?? 0}
                      onChange={(e) => {
                        const val = Math.max(0, Number(e.target.value));
                        setSelectedSeller(prev => ({ ...prev, boostedViews: val }));
                      }}
                      variant="outlined"
                      helperText="Added on top of real views. Seller sees the combined total."
                    />
                  </Box>
                </Grid>
                <Grid item xs={12}>
      ...
      >
        ...
        {adminCart.length === 0 ? (
          ...
          <>
            <List sx={{ mb: 2, maxHeight: 'calc(100dvh - 250px)', overflowY: 'auto' }}>
              {adminCart.map((item) => (
                <ListItem 
                  key={item.id}
                  secondaryAction={
