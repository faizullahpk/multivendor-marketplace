...
import { Box, Grid, Paper, Typography, useMediaQuery, useTheme, styled, alpha, Button, Dialog, DialogTitle, DialogContent, DialogActions, List, ListItem, ListItemText, CircularProgress, TextField, InputAdornment } from '@mui/material';
...
import { initSellerPresence, cleanupSellerPresence } from '../../utils/sellerPresence';
...
const ChatContainer = styled(Box)(({ theme }) => ({
  ...
  [theme.breakpoints.down('sm')]: {
    ...
    // Let the parent (SellerDashboard/AdminDashboard) control height
    height: '100%',
    maxHeight: '100%',
  }
}));
...
const Chat = ({ isAdmin, onMessageSent, onNavigateBack }) => {
  const [chats, setChats] = useState([]);
  const [selectedChatId, setSelectedChatId] = useState(null);
  const [loading, setLoading] = useState(true);
  // On mobile admin: start showing the list (true = list visible). On desktop this isn't used.
  ...
  // ── Seller presence: mark online when seller is active ──────────────────
  useEffect(() => {
    if (!currentUserUid || isAdmin) return;
    // initSellerPresence returns a cleanup function
    const cleanup = initSellerPresence(currentUserUid);
    return cleanup;
          ...
          if (!isAdmin && otherUserUid) {
            ...
            if (userDoc.exists()) {
              ...
              if (isAdmin) {
                ...
                // Capture seller presence for ChatList indicators
                sellerIsOnline = userData.isOnline || false;
                sellerLastSeen = userData.lastSeen || null;
              } else {
  ...
  const handleSelectChat = (chatId) => {
    setSelectedChatId(chatId);
    // sidebarOpen state not needed in new list-first pattern, but kept for safety
    if (isMobile) setSidebarOpen(false);
  };
  
  // Handle back button: admin → back to list; seller → back to dashboard tab (not re-navigate)
  const handleBackClick = () => {
    if (!isAdmin) {
      // Use the callback from SellerDashboard to switch tabs without pushing a new history entry.
      // Pushing navigate('/seller/dashboard') when already on that URL causes Android browser
      // back button to loop infinitely — the callback approach avoids that entirely.
  ...
  // Mobile: list-first pattern (WhatsApp/Telegram style)
  const renderChatLayout = () => {
    if (isMobile) {
      // ── ADMIN mobile: show list first, then window ──────────────────────
      if (isAdmin) {
        // No chat selected → full-screen chat list
        if (!selectedChatId) {
          return (
            <Box sx={{ width: '100%', height: '100%', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
              ...
              {/* Chat list */}
              <ScrollableChatListContainer sx={{ flex: 1 }}>
                {loading ? (
                  <Box sx={{ display: 'flex', justifyContent: 'center', pt: 6 }}>
                    <Typography color="text.secondary">Loading…</Typography>
                  </Box>
                ) : chats.length === 0 ? (
                  <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', pt: 8, px: 3 }}>
                    <Typography variant="body1" color="text.secondary" align="center">No conversations yet</Typography>
                    <Button
                      startIcon={<AddIcon />}
                      variant="outlined"
                      color="primary"
        ...
        // Chat IS selected → full-screen ChatWindow with back button
        return (
      ...
      // ── SELLER mobile: go straight to ChatWindow ────────────────────────
      return (
        <Box sx={{
          width: '100%',
          height: '100%',
