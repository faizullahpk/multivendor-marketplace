...
const CustomerDashboard = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      const fetchCustomerProfile = async () => {
        const uid = auth.currentUser?.uid || localStorage.getItem('customerId');
        const userEmail = auth.currentUser?.email || localStorage.getItem('customerEmail');
        if (uid) {
          try {
  ...
  const handleEmailChange = async () => {
    try {
      ...
      // Reauthenticate user
        if (!auth.currentUser) throw new Error('Session expired. Please log in again to change email.');
  ...
  const handlePasswordChange = async () => {
    try {
      ...
      // Reauthenticate user
          if (!auth.currentUser) throw new Error('Session expired. Please log in again to change password.');
          const credential = EmailAuthProvider.credential(
            auth.currentUser.email,
      ...
      >
        ...
        <List component="nav" sx={{ px: 2 }}>
          {[
            ...
            { label: 'Products', icon: <ShoppingBag />, index: 1 },
            { label: 'Shopping Cart', icon: <ShoppingCart />, index: 2, badge: cart.reduce((t, i) => t + i.quantity, 0) },
            { label: 'My Orders', icon: <ReceiptLong />, index: 3, chip: orders.length },
            { label: 'Manage Profile', icon: <Person />, index: 4 },
          ].map(({ label, icon, index, badge, chip }) => (
            <ListItemButton
              key={index}
            ...
            >
              ...
              <ListItemText primary={label} primaryTypographyProps={{ fontWeight: activeTab === index ? 'bold' : 'normal' }} />
              {badge > 0 && (
  ...
  return (
    <Box sx={{ display: 'flex' }}>
      ...
      {isMobile && (
        ...
        >
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
            <Box sx={{ width: 22, height: 2, bgcolor: 'white', borderRadius: 1 }} />
            <Box sx={{ width: 22, height: 2, bgcolor: 'white', borderRadius: 1 }} />
            <Box sx={{ width: 22, height: 2, bgcolor: 'white', borderRadius: 1 }} />
          </Box>
        </IconButton>
      ...
      {isMobile ? (
        ...
        /* Desktop: permanent drawer (always visible, pushes content) */
        <StyledDrawer
          variant="permanent"
          anchor="left"
      ...
      <Main
        ...
        sx={{
          ...
          height: '100vh',
          width: '100%',
          // On desktop, account for the permanent sidebar width
          ml: { sm: 0 },
        }}
      >
        <Box
