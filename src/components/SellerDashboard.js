...
const SellerDashboard = ({ setIsSeller }) => {
  ...
  // Credit Score & Star Rating - live from Firestore
  const [creditScore, setCreditScore] = useState(0);
  const [starRating, setStarRating] = useState(0);

  // Today Views (real + admin-boosted) - live from Firestore
  const [todayViews, setTodayViews] = useState(0);
  const [boostedViews, setBoostedViews] = useState(0);
  ...
  // Real-time listener for creditScore, starRating, and views from Firestore
  useEffect(() => {
    const sellerId = localStorage.getItem('sellerId');
    if (!sellerId) return;
    ...
    // ── Increment today's view count (once per dashboard session) ──────────
    const trackView = async () => {
      try {
        ...
        const d = snap.data();
        if (d.viewsDate === todayStr) {
          // Same day — just increment
          await updateDoc(sellerRef, { todayViews: (d.todayViews || 0) + 1 });
        } else {
          // New day — reset counter
          await updateDoc(sellerRef, { todayViews: 1, viewsDate: todayStr });
        }
    ...
    const unsubscribe = onSnapshot(sellerRef, (docSnap) => {
      if (docSnap.exists()) {
        ...
        setStarRating(data.starRating || data.rating || 0);
        setTodayViews(data.todayViews || 0);
        setBoostedViews(data.boostedViews || 0);
      }
    });
  ...
  // ── Android / mobile back-button fix for conversations tab ──────────────────
  // When the seller enters the conversations tab we push a sentinel history entry
  // so the device back button pops it instead of leaving the app.  When the pop
  // fires we simply switch back to the main dashboard tab without navigating away.
  useEffect(() => {
    if (activeTab !== 'conversations') return;

    // Push a dummy state so the browser history stack has something to pop.
    window.history.pushState({ sellerChatOpen: true }, '');

    const handlePopState = (e) => {
      // Device back was pressed while in conversations → go back to dashboard tab
  ...
  // Auto-save payment method toggles immediately without needing Edit mode
  const handlePaymentMethodChange = async (field, value) => {
    handleSettingsChange(field, value);
    try {
      const sellerId = localStorage.getItem('sellerId');
      if (!sellerId) return;
      const sellerRef = doc(db, 'sellers', sellerId);
      await updateDoc(sellerRef, { [field]: value });
    } catch (err) {
      console.error('Error auto-saving payment method:', err);
    }
  };
        ...
        <>
          ...
          <Grid container spacing={3} sx={{ mb: 3 }}>
            ...
            <Grid item xs={12} sm={6}>
              ...
              >
                <Box>
                  <Typography variant="body2" sx={{ opacity: 0.85, mb: 0.5, fontWeight: 500, letterSpacing: 1, textTransform: "uppercase", fontSize: "0.7rem" }}>
                    Credit Score
                  </Typography>
                    ...
                    >
                      <Box
                        sx={{
                          ...
                          top: 0,
                          height: "100%",
                          width: `${Math.min(Math.max((creditScore / 1000) * 100, 0), 100)}%`,
                          borderRadius: 3,
                          bgcolor: creditScore >= 700 ? "#69f0ae" : creditScore >= 400 ? "#ffeb3b" : "#ff5252",
                  ...
                  <Typography variant="caption" sx={{
                    ...
                    display: "block",
                    fontWeight: 600,
                    color: creditScore >= 700 ? "#69f0ae" : creditScore >= 400 ? "#ffeb3b" : "#ff5252",
                  }}>
                    {creditScore >= 700 ? "★ Excellent" : creditScore >= 400 ? "▲ Good" : "▼ Needs Improvement"}
                  </Typography>
                </Box>
                <TrendingUpIcon sx={{ fontSize: 64, opacity: 0.2, position: "absolute", right: 16, bottom: 16 }} />
                <TrophyIcon sx={{ fontSize: 48, opacity: 0.9 }} />
              </Paper>
            </Grid>
                ...
                <Box>
                  ...
                  <Box sx={{ mt: 1 }}>
                    <Rating
                      ...
                      size="medium"
                      sx={{
                        "& .MuiRating-iconFilled": { color: "#fff176" },
                        "& .MuiRating-iconEmpty": { color: "rgba(255,255,255,0.4)" },
                        "& .MuiRating-iconHalf": { color: "#fff176" },
                      }}
                    />
                  </Box>
                  <Typography variant="caption" sx={{ opacity: 0.85, fontWeight: 600, display: "block", mt: 0.5 }}>
                    {starRating >= 4.5 ? "★ Top Rated Seller" : starRating >= 3 ? "▲ Trusted Seller" : starRating > 0 ? "Building Reputation" : "Not yet rated"}
                  </Typography>
                </Box>
                <StarsIcon sx={{ fontSize: 64, opacity: 0.15, position: "absolute", right: 16, bottom: 16 }} />
                <StarsIcon sx={{ fontSize: 48, opacity: 0.9 }} />
              </Paper>
            </Grid>
          </Grid>
          ...
          <Grid container spacing={2} sx={{ mt: 2 }}>
            {/* Sales Stat Box — full-width beautiful chart */}
            <Grid item xs={12}>
              <Paper
                elevation={4}
                sx={{
              ...
              >
                ...
                <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
                  <Box>
                    ...
                    <Typography variant="body2" sx={{ color: 'rgba(255,255,255,0.7)', mt: 0.5 }}>
                      Order status breakdown for this month
                    </Typography>
                  </Box>
                  <Box sx={{
                    ...
                    <Typography variant="h4" sx={{ fontWeight: 800, color: '#69f0ae' }}>
                      ${Number(sellerData?.totalSales || 0).toFixed(2)}
                    </Typography>
                ...
                {(() => {
                  const bars = [
                    { label: 'New', value: newOrdersThisMonth, color: '#42a5f5', icon: '🛒' },
                    { label: 'Cancelled', value: cancelledOrdersThisMonth, color: '#ef5350', icon: '❌' },
                    { label: 'On Way', value: sellerOrders.filter(o => o.status === 'on-the-way').length, color: '#ffca28', icon: '🚚' },
                    { label: 'Picked', value: sellerOrders.filter(o => o.status === 'picked').length, color: '#ab47bc', icon: '📦' },
                    { label: 'Completed', value: completedOrdersThisMonth, color: '#66bb6a', icon: '✅' },
                  ];
                  const maxVal = Math.max(...bars.map(b => b.value), 1);
                  return (
                    <Box sx={{ display: 'flex', gap: { xs: 1, sm: 2 }, alignItems: 'flex-end', height: { xs: 120, sm: 160 }, mt: 1 }}>
                      {bars.map((bar) => (
                        <Box key={bar.label} sx={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 0.5 }}>
                          <Typography variant="caption" sx={{ color: 'white', fontWeight: 700, fontSize: { xs: '0.65rem', sm: '0.75rem' } }}>
                            {bar.value}
                          </Typography>
                          <Box sx={{
                            width: '100%',
                            height: `${Math.max((bar.value / maxVal) * (window.innerWidth < 600 ? 80 : 110), bar.value > 0 ? 8 : 4)}px`,
                            bgcolor: bar.color,
                            ...
                            minHeight: 4,
                            position: 'relative',
                            '&:hover': { opacity: 0.85, transform: 'scaleY(1.03)', transformOrigin: 'bottom' }
                          }} />
                          <Typography variant="caption" sx={{ color: 'rgba(255,255,255,0.8)', fontSize: { xs: '0.55rem', sm: '0.7rem' }, textAlign: 'center' }}>
                            {bar.icon} {bar.label}
                          </Typography>
                        </Box>
                      ))}
                ...
                <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap', mt: 2 }}>
                  {[
                    { label: `${sellerOrders.length} Total Orders`, color: '#42a5f5' },
                    { label: `${sellerData?.totalProducts || 0} Products`, color: '#66bb6a' },
                    { label: `${sellerOrders.filter(o => o.status === 'picked').length} Picked`, color: '#ab47bc' },
                  ].map(chip => (
                    <Box key={chip.label} sx={{
                      ...
                      <Typography variant="caption" sx={{ color: chip.color, fontWeight: 600, fontSize: '0.7rem' }}>
                        {chip.label}
                      </Typography>
                    </Box>
                  ))}
  ...
  const renderProductsContent = () => (
    <Container maxWidth={false} sx={{ mt: 2, width: '100%', px: { xs: 1.5, sm: 3, md: 4 }, overflowX: 'hidden' }}>
      ...
      <Paper elevation={3} sx={{ p: 2, px: { xs: 1, sm: 2, md: 3 }, width: '100%', overflow: 'hidden', boxSizing: 'border-box' }}>
  ...
  const renderProfileContent = () => (
    <Container maxWidth="lg" sx={{ mt: 4, px: { xs: 1, sm: 2, md: 3 } }}>
      ...
      <Paper elevation={3} sx={{ p: { xs: 2, sm: 3 }, overflowX: "hidden" }}>
        <Box
          sx={{
            display: "flex",
            justifyContent: "space-between",
              ...
              <FormControlLabel
                control={
                  <Checkbox
                    ...
                    onChange={(e) =>
                      handlePaymentMethodChange("bankPayment", e.target.checked)
                    }
                  />
                }
                label="Bank Payment"
              ...
              <FormControlLabel
                control={
                  <Checkbox
                    ...
                    onChange={(e) =>
                      handlePaymentMethodChange("usdtPayment", e.target.checked)
                    }
                  />
                }
                label="USDT Payment"
  ...
  const renderContent = () => {
    ...
    if (activeTab === "dashboard") {
      ...
      } else if (activeSubTab === "pickedOrders") {
        return renderPickedOrdersContent();
  ...
  const renderPickedOrdersContent = () => {
    const pickedOrders = sellerOrders.filter(
    ...
    return (
      <Box sx={{ p: { xs: 1, sm: 3 } }}>
        ...
        <Typography variant="body2" color="text.secondary" sx={{ mb: 3 }}>
          Orders that have been picked and are ready for dispatch
        </Typography>
        {pickedOrders.length > 0 ? (
          <Box sx={{ overflowX: 'auto', width: '100%' }}>
            {renderOrdersTable(pickedOrders)}
          </Box>
        ) : (
          <Paper sx={{ p: 4, textAlign: "center", borderRadius: 2 }}>
            ...
            <Typography variant="body2" color="text.secondary">
              Orders that have been marked as "Picked" will appear here.
            </Typography>
          </Paper>
        )}
      </Box>
...
const isMobile = useMediaQuery('(max-width:768px)');
  ...
  return (
    <Box sx={{ display: "flex", height: "100dvh", overflow: "hidden" }}>
      ...
      {/* Hamburger toggle button - only on mobile, hidden during conversations to avoid overlap */}
      ...
      <Box
        ...
        sx={{
          ...
          height: '100%',
          width: drawerWidth,
          background: 'linear-gradient(to bottom, #FF4D33, #FF5E46, #FF6E59)',
          color: 'white',
          overflowY: 'auto',
          overflowX: 'hidden',
          display: 'flex',
          ...
          transform: isMobile
            ? toogle ? 'translateX(0)' : `translateX(-${drawerWidth}px)`
            : 'translateX(0)',
          transition: 'transform 0.3s ease',
        }}
      >
          ...
          {[
            ...
            { icon: <ProductIcon />, text: "Products", value: "products" },
            {
              ...
              subItems: [
                ...
                { text: "Picked Up", value: "pickedOrders" },
              ],
            },
            {
              icon: <WithdrawIcon />,
      ...
      <Box
        ...
        sx={{
          flexGrow: 1,
          // No padding when conversations is active — chat fills the full space
          p: activeTab === 'conversations' ? 0 : { xs: 1, sm: 2, md: 3 },
          paddingTop: activeTab === 'conversations'
            ? { xs: '48px', sm: 0 }          // small offset for hamburger on mobile
            : { xs: '56px', sm: '16px', md: '16px' },
          marginLeft: { xs: 0, md: `${drawerWidth}px` },
          width: { xs: '100%', md: `calc(100% - ${drawerWidth}px)` },
          maxWidth: { xs: '100%', md: `calc(100% - ${drawerWidth}px)` },
          // For conversations: fixed-height container so chat doesn't overflow
          height: activeTab === 'conversations' ? '100%' : 'auto',
          minHeight: activeTab === 'conversations' ? 'unset' : '100dvh',
          background: activeTab === 'conversations'
            ? 'transparent'
            : 'linear-gradient(to bottom, #f5f7ff, #ffffff)',
          overflowX: 'hidden',
          overflowY: activeTab === 'conversations' ? 'hidden' : 'auto',
